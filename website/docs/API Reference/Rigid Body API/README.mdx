...
- **Named exports**: the xxx called before plugins are initialized.
...
`options` are the xxx. `options` are plugin-specific and are specified by users when they use them in `docusaurus.config.js`. If there's a xxx function exported, the `options` will be validated and normalized beforehand.
